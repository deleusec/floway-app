...
const StepOne: React.FC<StepOneProps> = ({ onNext }) => {
  ...
  const handleNext = () => {
    if (goalType === 'distance') {
      onNext(
        name,
        goalType,
        `${kilometers.padStart(2, '0')}.${meters.padStart(2, '0')}`,
      );
  ...
  return (
    <ScrollView>
      ...
      >
        <Text style={tw`font-poppins text-xl text-gray-800 mb-5`}>Name your soundscape</Text>
        <TextInput
          style={tw`font-poppins border border-gray-400 rounded-lg py-2 px-4 w-full mb-5`}
          value={name}
          onChangeText={setName}
        />
        <Text style={tw`font-poppins text-lg text-gray-700 mb-5`}>
        ...
        <View style={tw`flex-row mb-5`}>
          <Pressable
            style={[
              ...
              goalType === 'distance' ? tw`bg-primary` : tw`bg-blue-300`,
            ]}
            onPress={() => setGoalType('distance')}
          >
            <Text style={tw`font-poppins text-white`}>Distance</Text>
          </Pressable>
          <Pressable
            style={[
              tw`py-2 px-4 mx-2 rounded-lg`,
              goalType === 'time' ? tw`bg-primary` : tw`bg-blue-300`,
            ]}
            onPress={() => setGoalType('time')}
          >
            <Text style={tw`font-poppins text-white`}>Time</Text>
          </Pressable>
        </View>
        {goalType === 'distance' && (
        ...
        >
          <Text style={tw`font-poppins text-white`}>Next</Text>
        </Pressable>
      </View>
    </ScrollView>
  );
